...
var firmwareTemplate = '<tr class="category_{ $CATEGORY }"><th scope="row">{ $INDEX }</th><td>{ $VERSION }</td><td>{ $REGION }</td><td>{ $RELEASE_DATE }</td><td><a href="javascript:;" device="{ $DEVICE }" changelogID="{ $CHANGELOG_ID }" category="{ $CATEGORY }" class="showChangelog">Show changelog</a></td><td><a href="{ $URL }">Download</a></td>';
var changelogs = [];
